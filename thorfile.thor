...
class A < Thor
	...
	desc 'new_tpl name[,desc[,force]]', 'create a css file for module'
	def new_xtpl path, desc, force = false
		code = template(:xtemplate){|ctx| ctx[:desc] = desc}
		try_write_file(path.as_file('.xtemplate'), "/* #{desc}*/", force){|js|edit_it? js}
	end
